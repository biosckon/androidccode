...
size_t writecb(char *content, size_t size, 
		size_t nmemb, void *userp) {
    size_t actsize = size * nmemb;

    mem_struct *mem = (mem_struct *) userp;
    char *newmem = realloc(
			mem->memory, mem->size + actsize + 1);
    if (newmem == NULL) {
...
int main() {
    ...
    res = curl_easy_setopt(hand, 
			CURLOPT_FOLLOWLOCATION, enable);
    res = curl_easy_setopt(hand, 
    ...
    do {
        res = curl_multi_wait(
				mhand, NULL, 0, 1000, NULL);
        res = curl_multi_perform(
				mhand, &transfer_run);
    } while (transfer_run);
